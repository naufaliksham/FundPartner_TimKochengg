    ...
    <div class="centered">
        <div class="card" style="width: 400px;">
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('pembayaran', $detail->id) }}">
                    @csrf
                    
                    @php
                        // $tempo = $detail->tanggal_jatuh_tempo;
                        $today = now()->format('Y-m-d');
                        $fee = round($detail->dana * 0.05, 2);
                        // dd($today, $tempo);
                        // if ($today > $tempo) {
                            // $test = true;
                        ...
                        //     $denda = 0;
                        // }
                        $total = $detail->dana + $fee;
                        // dd($denda);
                        @endphp
                    <div style="display: flex; justify-content: space-between; align-items: baseline;">
                        <h5 style="text-align: left; margin-bottom: 0;">Saldo:
                            Rp.{{ Auth::user()->saldo }}</h5>
                            <h5 style="text-align: left; margin-bottom: 0;">{{ '$detail->tanggal_jatuh_tempo' }}</h5>
                    </div>
                    <hr>
                    <table style="width: 100%;">
                        <tr>
                            ...
                            <td style="text-align: right;">
                                Rp.{{ number_format($detail->dana, 0, ',', '.') ?? '-' }}</td>
                                <input type="hidden" name="jumlah" value="{{ $detail->dana }}">
                            </tr>
                            {{-- <tr>
                            ...
                            <tr>
                                <th style="text-align: left;">detail aplikasi(5%)</th>
                                <td></td>
                                <td></td>
                            ...
                            <tr>
                                ...
                                <td></td>
                                <td></td>
                                <td style="text-align: right;">{{ $detail->pembayaran }}</td>
                                <input type="hidden" name="jenis" value="{{ $detail->pembayaran }}">
                        </tr>
                        <tr>
                    ...
                    <div class="text-center">
                        ...
                        @if (Auth::user()->saldo >= $detail->jumlah_pembayaran)
